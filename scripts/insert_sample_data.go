...
func main() {
	...
	dsn := "postgres://setu_gateway:setu_gateway1324!!@172.16.100.250:5432/setu_gateway?sslmode=disable"
	...
	`
	_, err = pool.Exec(ctx, upstreamsSQL)
	if err != nil {
		log.Printf("Warning: Failed to insert upstreams: %v\n", err)
	} else {
		fmt.Println("  ✅ 4 upstreams inserted")
	}
	...
	`
	_, err = pool.Exec(ctx, targetsSQL)
	if err != nil {
		log.Printf("Warning: Failed to insert targets: %v\n", err)
	} else {
		fmt.Println("  ✅ 6 targets inserted")
	}
	...
	routesSQL := `
		...
		ON CONFLICT DO NOTHING
	`
	_, err = pool.Exec(ctx, routesSQL)
	if err != nil {
		log.Printf("Warning: Failed to insert routes: %v\n", err)
	} else {
		fmt.Println("  ✅ 9 routes inserted")
	}

	// Insert rate limits
	fmt.Println("\n📝 Inserting rate limits...")
	rateLimitsSQL := `
		...
		ON CONFLICT DO NOTHING
	`
	_, err = pool.Exec(ctx, rateLimitsSQL)
	if err != nil {
		log.Printf("Warning: Failed to insert rate limits: %v\n", err)
	} else {
		fmt.Println("  ✅ 3 rate limits inserted")
	}

	// Verify data
	fmt.Println("\n📊 Verification:")
	
	var upstreamCount, targetCount, routeCount, rateLimitCount int
	
	pool.QueryRow(ctx, "SELECT COUNT(*) FROM upstreams").Scan(&upstreamCount)
	fmt.Printf("  • Upstreams: %d\n", upstreamCount)
	
	pool.QueryRow(ctx, "SELECT COUNT(*) FROM targets").Scan(&targetCount)
	fmt.Printf("  • Targets: %d\n", targetCount)
	
	pool.QueryRow(ctx, "SELECT COUNT(*) FROM routes").Scan(&routeCount)
	fmt.Printf("  • Routes: %d\n", routeCount)
	
	pool.QueryRow(ctx, "SELECT COUNT(*) FROM rate_limits").Scan(&rateLimitCount)
	fmt.Printf("  • Rate Limits: %d\n", rateLimitCount)

	fmt.Println("\n✅ Sample data insertion complete!")
	fmt.Println("\n🚀 Now restart the gateway to load the new routes:")
	fmt.Println("   .\\setu-gateway.exe")
	fmt.Println("\n📡 Then test with:")
	fmt.Println("   Invoke-WebRequest -Uri 'http://localhost:8080/api/users' -UseBasicParsing")
	fmt.Println("   Invoke-WebRequest -Uri 'http://localhost:8080/api/products' -UseBasicParsing")
	fmt.Println("   Invoke-WebRequest -Uri 'http://localhost:8080/health' -UseBasicParsing")
	fmt.Println("\n📈 Then check metrics:")
	fmt.Println("   Invoke-WebRequest -Uri 'http://localhost:9090/metrics' -UseBasicParsing")
}
